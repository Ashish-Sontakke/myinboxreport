...
export async function initGoogleAuth(): Promise<void> {
  ...
  tokenClient = google.accounts.oauth2.initTokenClient({
    ...
    error_callback: (error) => {
      // Fires when the popup is blocked, closed, or fails to open —
      // without this, a pending signIn() promise would hang forever.
      pendingReject?.(new Error(error.message || `Sign-in failed (${error.type})`));
      pendingResolve = null;
      pendingReject = null;
    },
  });
...
/**
 * Attempt a silent token refresh (no consent screen). Resolves to the new
 * access token, or null if silent auth failed — popup blocked, consent
 * required, ITP restrictions. Never rejects; callers fall back to signIn().
 */
export function signInSilent(): Promise<string | null> {
  if (!tokenClient) return Promise.resolve(null);

  return new Promise<string | null>((resolve) => {
    pendingResolve = (token) => resolve(token.access_token);
